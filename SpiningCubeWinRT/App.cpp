...
void SpiningCubeApp::Startup()
{
	...
	task<Utility::ByteArray> loadVSTask = HolographicEngine::Utility::ReadFileAsync(vertexShaderFileName);
	task<Utility::ByteArray> loadPSTask = HolographicEngine::Utility::ReadFileAsync(L"ms-appx:///PixelShader.cso");

	task<Utility::ByteArray> loadGSTask;

	if (!m_usingVprtShaders)
	{
		// Load the pass-through geometry shader.
		loadGSTask = HolographicEngine::Utility::ReadFileAsync(L"ms-appx:///GeometryShader.cso");
	}

	// After the vertex shader file is loaded, create the shader and input layout.
	task<void> createVSTask = loadVSTask.then([this](const Utility::ByteArray& fileData)
	{
		winrt::check_hresult(Graphics::g_Device->CreateVertexShader(fileData->data(), fileData->size(), nullptr, &m_vertexShader ) );
		...
		winrt::check_hresult(Graphics::g_Device->CreateInputLayout(vertexDesc.data(), static_cast<UINT>(vertexDesc.size()), fileData->data(),
	...
	task<void> createPSTask = loadPSTask.then([this](const Utility::ByteArray& fileData)
	{
		winrt::check_hresult(
			Graphics::g_Device->CreatePixelShader(
				fileData->data(),
				fileData->size(),
				nullptr,
				&m_pixelShader
