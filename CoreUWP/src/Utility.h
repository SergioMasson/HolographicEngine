...
#define ASSERT( isFalse, ... ) \
        if (!(bool)(isFalse)) { \
            HolographicEngine::Utility::Print("\nAssertion failed in " STRINGIFY_BUILTIN(__FILE__) " @ " STRINGIFY_BUILTIN(__LINE__) "\n"); \
            HolographicEngine::Utility::PrintSubMessage("\'" #isFalse "\' is false"); \
            HolographicEngine::Utility::PrintSubMessage(__VA_ARGS__); \
            HolographicEngine::Utility::Print("\n"); \
            __debugbreak(); \
        }
...
#define ASSERT_SUCCEEDED( hr, ... ) \
        if (FAILED(hr)) { \
            ...
            HolographicEngine::Utility::PrintSubMessage("hr = 0x%08X", hr); \
            HolographicEngine::Utility::PrintSubMessage(__VA_ARGS__); \
            HolographicEngine::Utility::Print("\n"); \
            __debugbreak(); \
        }
...
#define WARN_ONCE_IF( isTrue, ... ) \
    { \
        ...
        if ((bool)(isTrue) && !s_TriggeredWarning) { \
            ...
            HolographicEngine::Utility::PrintSubMessage("\'" #isTrue "\' is true"); \
            HolographicEngine::Utility::PrintSubMessage(__VA_ARGS__); \
            HolographicEngine::Utility::Print("\n"); \
        } \
    }
...
#define DEBUGPRINT( msg, ... ) \
    HolographicEngine::Utility::Printf( msg "\n", ##__VA_ARGS__ );
